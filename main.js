...
const PF_DATA = {
  ...
  midnight: {
    title: "MIDNIGHT - MUSINSA<br/>클론코딩 리디자인",
    cardTitle: "MIDNIGHT",
    name: "메인 페이지 클론 코딩",
    created: "2026/06/08 → 2026/06/22",
    type: [
      ["Visual Studio Code", "#007acc"],
      ["ChatGPT", "#10a37f"],
      ["FIGMA", "#6b4a8b"],
    ...
    overview:
      "기존 무신사의 대중적이고 복잡한 커머스 레이아웃을 '미드나잇(Midnight)'이라는 명확한 컨셉에 맞춰 힙하고 미니멀한 패션 웹진 형태로 리디자인",
    work: "• 다크 모드 컨셉 적용: 밝고 복잡했던 기존 톤을 밤 시간대의 거친 콘크리트 지하보도 느낌을 주는 어둡고 깊이 있는 '다크 모드' 비주얼로 전면 전환\n• 오버사이즈/테크웨어 감성 반영: 미니멀하고 스트리트한 패션 감성에 맞춰 대형 배너와 모델 착장 이미지가 돋보이도록 그리드(Grid) 레이아웃을 커스텀\n• UI 요소 미니멀화: 화면을 빽빽하게 채우던 텍스트와 상품 나열을 과감히 줄이고, 웹사이트 UI가 돋보일 수 있도록 깔끔하고 정돈된 화면 구조를 설계\n• 퍼블리싱 커스터마이징: 바뀐 컨셉과 어두운 배경에 최적화되도록 직접 소스 코드를 수정하고 반응형 웹 퍼블리싱을 완성",
    team: "본인 1명 (1인 총괄 프로젝트)",
    result:
      "• 독창적인 브랜드 아이덴티티 구축: 흔한 쇼핑몰 스타일에서 벗어나 패션 브랜드 고유의 감성과 스트리트 무드를 극대화한 독창적인 레이아웃 완성\n• 시각적 몰입도 향상: 불필요한 시선 분산 요소를 제거하고 드라마틱한 그림자와 조명 효과가 느껴지는 미디어를 전면에 배치해 가독성과 몰입감을 높임\n• 컨셉 맞춤형 구현 능력 입증: 단순한 리디자인을 넘어 특정 타깃과 명확한 패션 컨셉에 맞춘 UI/UX 기획 및 퍼블리싱 역량을 증명",
  },
  "changwon-dark": {
